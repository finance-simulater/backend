...
출력된다. 기본적으로 같은 이메일에는 60초에 한 번만 전송할 수 있다.
...
서버가 이메일과 임시 티켓의 조합을 확인한 뒤에만 인증 완료 상태의 사용자를
생성한다. 성공하면 access token을 응답하고 refresh token 쿠키를 설정하며,
